...
#library(JASPAR2020, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
#library(TFBSTools, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
#library(motifmatchr, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
#library(BiocManager, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
#library(rtracklayer, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
#library(Biostrings, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
#library(BSgenome, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
#library(BSgenome.Hsapiens.NCBI.GRCh38, lib.loc = "/Shared/IVR/apvoigt/programs/R-4.1.0/bin/PACKAGES/")
...
if(my_step == 1){
  ...
  for(i in 1:length(peaks_list)){
    print(i)
    my_peaks <- read.table(
      file = paste0(my_filepaths[i,1], "/outs/peaks.bed"),
  ...
  for(i in 1:length(fragments_list)){
    print(i)
    md <- read.table(
      file = paste0(my_filepaths[i,1], "/outs/singlecell.csv"),
    ...
    counts <- Read10X_h5(filename = paste0(my_filepaths[i,1], "/outs/filtered_peak_bc_matrix.h5"))
    md <- md[which(rownames(md) %in% colnames(counts)), ]

    frags <- CreateFragmentObject(
      path = paste0(my_filepaths[i,1], "/outs/fragments.tsv.gz"),
      cells = rownames(md)
    )
  ...
  for(i in 1:length(objects_list)){
    ...
    seurat_obj <- CreateSeuratObject(object_assay,
                                     assay = "ATAC",
                                     meta.data = md,
                                     min.features = 100 ## DV added. I think this is good but check w/ Nate
    )
  ...
  for(i in 1:length(objects_list)){
    ...
    # NKM added this
    seurat_obj$mitochondrial_ratio <- seurat_obj$mitochondrial/seurat_obj$total * 100
...
if(my_step == 2){
  ...
  for(i in 1:length(objects_list)){
    ...
    seurat_obj_filtered <- subset(
      x = seurat_obj,
      subset = peak_region_fragments > peak_region_fragments_lower &
        peak_region_fragments < peak_region_fragments_upper &
  ...
  # genome(BSgenome.Hsapiens.NCBI.GRCh38) <- "GRCh38"
  #
  ...
  save(save_list, paste0(my_save_path, "/", current_date, "-run-commands.RData"))
